...
var zoom = d3.behavior.zoom().on("zoom", zoomed).scaleExtent([1, 100]);
...
function display(min_max_freq_on_each_card) {
  ...
  var sectors = svgContainer.selectAll(".blocks").data([curr_set_to_display]);
  ...
  path
    .startAngle(function (d) {
      return d.data.startAngle;
    })
    .endAngle(function (d) {
      return d.data.endAngle;
    })
  ...
  var arc_paths = sectors.selectAll(".blocks .slice").data(pie, function (d) {
  ...
  var exit = d3
    ...
    .duration(0)
    .each(function () {
      arc_paths.exit().transition().remove();
    });
    ...
    .each(function () {
      arc_paths
        ...
        .attr("fill", function (d, i) {
          ...
          var thisMinFreq =
            min_max_freq_on_each_card[thisCardinality - min_card][0];
          var thisMaxFreq =
            min_max_freq_on_each_card[thisCardinality - min_card][1];
  ...
  var enter = update
    ...
    .duration(0)
    .each(function () {
      var curr_arc_paths = arc_paths
        .enter()
        .append("path")
        .attr("class", "slice")
        .attr("data-itemset", function (d, i) {
          return curr_set_to_display[i].itemset;
        })
        .attr("data-frequency", function (d, i) {
          return curr_set_to_display[i].frequency;
        })
        .attr("fill", function (d, i) {
          freqSet.add(curr_set_to_display[i].frequency);
      ...
      curr_arc_paths.transition().attr("d", path);

      curr_arc_paths.append("title").text(function (d, i) {
        var displaySet = curr_set_to_display[i].itemset.slice(
          curr_set_to_display[i].rank - 1,
          curr_set_to_display[i].itemset.length
        );

        return (
          "{" +
          curr_set_to_display[i].itemset +
          "}: " +
          curr_set_to_display[i].frequency
        );
      });
    });
...
function set_angles(arr, parentStart, parentEnd, totalWeight) {
  ...
  for (var i = 0; i < arr.length; i++) {
    ...
    arr[i].endAngle =
      arr[i].startAngle +
      (Math.abs(parentEnd) - Math.abs(parentStart)) * ratio * -1;
    ...
    if (arr[i].children.length > 0) {
      ...
      set_angles(
        ...
        arr[i].endAngle,
        nextTotalWeight
      ); // <---- this is where you tell it how to divide the sectors among descendant children
    }
  }
}
